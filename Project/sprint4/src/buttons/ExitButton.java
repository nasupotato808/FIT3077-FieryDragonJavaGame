...
public class ExitButton extends JPanel implements MouseListener {
    ...
    public void mouseClicked(MouseEvent e) {
        int result = JOptionPane.showConfirmDialog(parent, "Do you want to save your progress before exiting?", "Exit", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE, null);
        if (result == JOptionPane.YES_OPTION){
            JFileChooser fileChooser = new JFileChooser(new File(System.getProperty("user.dir") + "/Project/sprint4/src/saves"));
            fileChooser.setSelectedFile(new File("game_save.json"));
            int option = fileChooser.showSaveDialog(parent);
            if (option == JFileChooser.APPROVE_OPTION) {
                String filePath = fileChooser.getSelectedFile().getPath();
                ((Game) parent).saveGame(filePath);
            }
            parent.dispose();
            new HomePage();
