...
public class Game extends JFrame implements Runnable{
    ...
    /**
     * Save the game progress to json file.
     *
    ...
    public void saveGame(String filePath) {
        ...
        File savesDir = new File(System.getProperty("user.dir") + "/Project/sprint4/src/saves");
        if (!savesDir.exists()) {
            savesDir.mkdirs();
        }

        // Create a new File object
        File saveFile = new File(savesDir, new File(filePath).getName());

        // Save the game state to the specified file
        GameState gameState = new GameState(players, names, completeMap, currentToken);
     ...
     * @param filePath a string file path
     *
     * @return the current game state that has been loaded
     */
